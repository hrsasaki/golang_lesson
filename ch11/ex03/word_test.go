...
// ex03
func randomStringNotParindrome(rng *rand.Rand) string {
	n := rng.Intn(23) + 2 // random length from 2 up to 24
	runes := make([]rune, n)
	for i := 0; i < (n+1)/2; i++ {
...
func TestRandomNotParindromes(t *testing.T) {
	// Initialize a pseudo-random number generator.
	seed := time.Now().UTC().UnixNano()
	t.Logf("Random seed: %d", seed)
	rng := rand.New(rand.NewSource(seed))
	...
	for i := 0; i < 1000; i++ {
		p := randomStringNotParindrome(rng)
		if IsPalindrome(p) {
