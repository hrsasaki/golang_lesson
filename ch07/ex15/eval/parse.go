...
//
func Parse(input string, inputsVars ...bool) (_ Expr, err error) {
	for _, inputsVariables := range inputsVars {
		inputs = inputsVariables
		// only first element
		break
	}
	defer func() {
...
func inputVar(name string) {
	fmt.Printf("Input value of variable %s:", name)
